...
def resolve_paths() -> tuple[Path, Path]:
    ...
    articles_root = (
        ...
        / "WEB"
        / "lichnost_i_telo"
        / "moya_samootsenka_i_identifikaciya"
        / "concepts"
    )
...
def load_link_map(script_dir: Path) -> dict[str, list[str]]:
    ...
    if not isinstance(data, dict):
        raise ValueError(
            'link_map.json должен содержать объект вида {"file.md": ["термин1", "термин2"]}'
        )
...
def run_processing(
    ...
    for article_path in md_files:
        old_content, new_content, replacements_log = process_article(
            article_path, articles_root, link_map
        )
